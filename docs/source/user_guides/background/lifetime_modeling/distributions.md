...
[Censoring and truncation](censoring_and_truncation.md):
...
Kaplan-Meier estimator (see [Non-parametric lifetime models](non_parametric_models.md)) is the
natural reference here: it makes no shape assumption, so a parametric curve that strays far
from it is a poor description of the data.
...
![Four fitted parametric survival functions compared with the Kaplan-Meier step function](../../../_static/plots/distributions_plot_1.png)
...
contributes a factor that depends on what was actually observed. Consider \(\theta\) for
the parameters, \(f\) for the probability density, \(S\) for the survival function,
\(H = -\log S\) for the cumulative hazard, \(\mathcal{D}\) for the set of observed
failures and \(\mathcal{C}\) for the right-censored ones, each entering the study at age
\(a_i\). A failure is informative about the density at \(t_i\); a right-censored unit
only tells us it was still alive at \(t_i\), hence \(S(t_i;\theta)\); and left truncation
conditions each factor on having survived up to the entry age \(a_i\), which is what the
...
that: with \(k\) parameters, \(n\) observations and \(\hat{\ell}\) the maximized
log-likelihood,
...
Covariates are the next step: see [Regressions: adding covariates](regressions.md) for models
where lifetime depends on the conditions each asset operates under.
