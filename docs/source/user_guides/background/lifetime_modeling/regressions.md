...
exposed to (see [Datasets](../../datasets.md)).

ReLife offers two parametric ways of letting a covariate vector \(x\) act on a baseline
cumulative hazard \(H_0\). Proportional hazard scales the hazard,
...
In both cases \(\beta\) is the vector of covariate coefficients, and both are fitted the
...
Two things follow. First, the ratio does not depend on \(t\): that is exactly what
...
Second, taking \(x_1\) and \(x_2\) equal everywhere except on one covariate raised by
one unit leaves
...
so \(e^{\beta_k}\) is the multiplicative effect on the instantaneous failure rate of a
one-unit increase in the \(k\)-th covariate, all others held fixed. Above 1 the asset
...
failure time form has no constant hazard ratio at all: there, \(e^{\beta \cdot x}\)
...
The likelihood is the one described in [Parametric distributions](distributions.md), with the
covariate effect substituted into \(H\). Nothing else changes:
...
The important point is that the coefficients \(\beta\) and the baseline parameters
\(\theta\) are estimated **jointly**, in a single optimization: the covariate effect you
...
observed failure times gives Cox's **partial** likelihood, where \(\mathcal{R}_j\) is the
risk set just prior to \(t_j\) (see
[Non-parametric lifetime models](non_parametric_models.md) for the at-risk
bookkeeping), \(\mathcal{D}_j\) the units failing at \(t_j\), and
\(d_j = |\mathcal{D}_j|\):
...
![Cox-estimated survival curves of two insulator strings with different acid exposure](../../../_static/plots/regressions_plot_1.png)
