...
Asset lifetime data is rarely a clean list of "this asset failed after \(t\) units of
...
approximately known. Any of these turns a simple "failed at time \(t\)" record into
...
For each asset, four dates matter: when the *observation window* opens (\(t_{start}\))
and closes (\(t_{end}\)), and when the asset itself was installed (\(t_{init}\)) and
failed (\(t_{fail}\), if that was actually observed). The figure below lines up six
assets on the same calendar to show every combination that can occur:

![Observation scheme for six assets, showing complete observations, right-censoring, left-censoring and left-truncation.](../../../_static/figures/observation_scheme.png)

> Observation scheme for six assets. A filled circle is an observed failure; an open circle
> means the asset was still working when the observation window closed at \(t_{end}\)
> (**right-censoring**); a right-pointing triangle before \(t_{start}\) means the asset was
> already installed before observation began (**left-truncation** if it fails after
> \(t_{start}\), or **left-censoring**, dashed, if a failure before \(t_{start}\) is only
> known to have happened, without knowing exactly when).

ReLife's `event`/`entry` pair covers the two cases that dominate industrial asset data
(right-censoring and left-truncation), which is why they are the two arguments widely used in
ReLife's functions. Note that left-censoring (asset 6 above) is rarer in practice and isn't
part of ReLife's data model.
...
The `load_power_transformer` dataset (see [Datasets](../../datasets.md)) has both kinds of rows:
...
![Survival curves and mean lifetimes under three assumptions about the same industrial dataset: 16, 38, and 46 years.](../../../_static/figures/censoring_bias.png)

> Same dataset, three assumptions. Counting only observed failures and ignoring the
> still-working assets (**mortality bias**) gives a mean lifetime of 16 years. Accounting for
> right-censoring but ignoring left-truncation (**survivor bias**) gives 46 years. Accounting
> for both gives 38 years: the correct figure lies in between, not at either extreme.
...
[Parametric distributions](distributions.md)), rather than requiring you to drop or
approximate either kind of observation.
