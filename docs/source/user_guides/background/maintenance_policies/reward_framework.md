...
[Run-to-failure policy](run_to_failure.md) and
[Preventive age replacement policy](preventive_age_replacement.md) both reduce to the same
question: attach a cost to each renewal of the [renewal process](renewal_theory.md), and work
out what that comes to per unit of time.
...
\(D\) is the **discounting factor**: a cost \(c\) paid at date \(x\) is worth
\(c\,D(x)\) today. \(AF\) is the **annuity factor**, the present value of paying 1
per unit of time from 0 to \(t\); dividing a cumulated cost by it gives the constant
...
(\(D = 1\) and \(AF(t) = t\)) and every cost counts in full whenever it occurs.
...
[Preventive age replacement policy](preventive_age_replacement.md).
...
of [Renewal processes and the renewal equation](renewal_theory.md), except that a renewal now
contributes its discounted cost instead of a unit count. With a final time and a number of
points, it returns the timeline together with the values:
...
of
[`asymptotic_expected_total_reward`](../../../api/stochastic_processes.md#relife.stochastic_processes.RenewalRewardProcess.asymptotic_expected_total_reward).
