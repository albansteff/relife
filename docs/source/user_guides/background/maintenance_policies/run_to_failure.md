...
`cf` per failure[^1]. There is no preventive action: it's the baseline every other
...
Under the hood, this wraps the fitted lifetime model in a
[renewal process](renewal_theory.md) where every renewal costs `cf`, i.e. a constant reward
\(Y = c_f\) (see [The reward and discounting framework](reward_framework.md)). The long-run
expected cost per unit of time follows directly from that: undiscounted, the renewal reward
theorem gives the expected cost of one cycle divided by its expected duration,
...
where \(X\) is the asset's lifetime and \(q(t)\) the expected equivalent annual cost
up to \(t\). This is what the policy reports:
...
decision variable. Compare with
[Preventive age replacement policy](preventive_age_replacement.md), where replacing *before*
...
[^1]:
    Van der Weide, J. A. M., & Van Noortwijk, J. M. (2008). Renewal theory with
